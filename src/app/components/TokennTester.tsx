...
export default function TokenTester() {
  ...
  const allowance = useReadContract({
    ...
    query: {
      enabled: Boolean(allowOwner || allowSpender) && Boolean(decimalsData),
    },
  })
  const allowanceText =
  ...
  async function handleTransfer() {
    if (!isAddress(transferTo))
      throw new Error('받는 주소가 올바르지 않습니다.')
  ...
  async function handleApprove() {
    if (!isAddress(approveSpender))
      throw new Error('spender 주소가 올바르지 않습니다.')
  ...
  async function handleTransferFrom() {
    ...
    if (!isAddress(transferFromTo))
      throw new Error('to 주소가 올바르지 않습니다.')
          ...
          <div className='grid gap-3'>
            ...
            <div className='rounded-xl border border-black/10 bg-zinc-50 p-3 text-sm dark:border-white/10 dark:bg-zinc-900/30'>
              <div className='flex items-center justify-between gap-3'>
                ...
                <div className='font-mono text-zinc-900 dark:text-zinc-100'>
                  {effectiveWatchAddress
                    ? shortAddr(effectiveWatchAddress)
                    : '—'}
                </div>
              </div>
              <div className='mt-2 flex items-center justify-between gap-3'>
